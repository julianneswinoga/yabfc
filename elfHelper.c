...
void setupELFHeader(Elf64_Ehdr *ELFHeader,
    ...
    for (int i = EI_PAD; i < EI_NIDENT; i++) {
        ELFHeader->e_ident[i] = 0x0;  // Zero padding
    }
    ...
    if (shnum > SHN_LORESERVE) {         // Number of entries in section header
        ...
        fprintf(stderr,
                "WARNING: Elf64_Ehdr.e_shnum is greater than SHN_LORESERVE, "
                "sh_size should be set to the number of entries in the program header table\n");
    } else {
        ELFHeader->e_shnum = shnum;
    }

    int e_shstrndx = SHN_UNDEF;  // Section header table index of the entry associated with the section name string
                                 // table, to be set later

    if (e_shstrndx > SHN_LORESERVE) {
        ELFHeader->e_shstrndx = SHN_XINDEX;
        fprintf(stderr,
                "WARNING: Elf64_Ehdr.e_shstrndx is greater than SHN_LORESERVE, "
