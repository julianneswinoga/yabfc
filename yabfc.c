...
static error_t parse_opt(int key, char *arg, struct argp_state *state) {
    ...
    switch (key) {  // Switch for setting the argument values
        ...
        case 's':
            ...
            break;
        case 'v':
            arguments->verbose = arg ? atoi(arg) : 2;
            break;
        case 'o':
            arguments->output_file = arg;
            break;
        case 'O':
            arguments->optimize = arg ? atoi(arg) : 2;
            break;
        ...
        case ARGP_KEY_END:
            if (state->arg_num < 1) {
                argp_usage(state);  // Not enough files
            }
            break;
...
static void compileFile(char *inputFileName) {
    char *outputFilename;
    FILE *readFile, *writeFile;            // Read and write file pointers
    readFile = fopen(inputFileName, "r");  // Open file for reading
    if (readFile == NULL) {
    ...
    if (strcmp(globalOptions.outputFile, "") != 0) {
        ...
        outputFilename = filenameWithoutExtension(inputFileName);
    ...
    INSTRUCTIONS instructions = {.size = 0, .instruction = malloc(0)};
    ...
    while ((readCharacter = fgetc(readFile)) != EOF) {
        if (strchr(VALID_COMMANDS, readCharacter) != NULL) {
            ...
            INSTRUCTION tempInstruction = {.type = readCharacter, .bracketMatch = -1};

            instructions.instruction =
                (INSTRUCTION *)realloc(instructions.instruction, (instructions.size + 1) * sizeof(INSTRUCTION));
    ...
    CODE code = {.size = 0, .bytes = malloc(0)};
    ...
    for (int i = 0; i < instructions.size; i++) {
        if (optimize_compress_PTR(&instructions, &i, &code) |  // Bitwise OR to prevent short circuit evaluation
            optimize_compress_ADDSUB(&instructions, &i, &code) | optimize_clear_loop(&instructions, &i, &code) |
        ...
        switch (instructions.instruction[i].type) {
            ...
            case ']':
                ...
                debugPrintf(3, "Closing bracket @ %i jumps backward by %i\n", i,
                            instructions.instruction[i].bracketMatch);
    ...
    setupSectionHeader(&sectionHeaderTable[3], /* .shrtrab header */
                       stringIndexFromSectionIndex(stringTable.bytes, 3), SHT_STRTAB, 0, 0,
                       TEXT_FILE_LOC + text.size + data.size, stringTable.size);

    ELFHeader.e_shstrndx = 3;  // Set which section header describes the string table

    ELFHeader.e_shoff = SEC_HEADER_NUM == 0 ? 0x0
                                            : TEXT_FILE_LOC + text.size + data.size +
                                                  stringTable.size;  // Finally determine the section header offset
...
int main(int argc, char *argv[]) {
    ...
    debugPrintf(1, "Optimization level: %i\nVerbosity level: %i\nSilent mode: %s\nOutput file: %s\n",
                globalOptions.optimize, globalOptions.verbose, globalOptions.silent ? "true" : "false",
    ...
    for (int i = 0; arguments.inputFiles[i]; i++) {  // Loop through the input files
        ...
        compileFile(arguments.inputFiles[i]);
    }
}
